...
size_t ppelib_iconv(ppelib_iconv_t cd, const char **inbuf, size_t *inbytesleft, char **outbuf, size_t *outbytesleft);

/* libiconv interface for vim */
#if defined(MAKE_DLL)
int
iconvctl (ppelib_iconv_t cd, int request, void* argument)
{
    /* not supported */
    return 0;
}
...
typedef ppelib_iconv_t (*f_iconv_open)(const char *tocode, const char *fromcode);
typedef int (*f_iconv_close)(ppelib_iconv_t cd);
typedef size_t (*f_iconv)(ppelib_iconv_t cd, const char **inbuf, size_t *inbytesleft, char **outbuf, size_t *outbytesleft);
...
static int win_iconv_close(ppelib_iconv_t cd);
static size_t win_iconv(ppelib_iconv_t cd, const char **inbuf, size_t *inbytesleft, char **outbuf, size_t *outbytesleft);
...
ppelib_iconv_t
ppelib_iconv_open(const char *tocode, const char *fromcode)
{
    rec_iconv_t *cd;

    cd = (rec_iconv_t *)calloc(1, sizeof(rec_iconv_t));
    if (cd == NULL)
        return (ppelib_iconv_t)(-1);

#if defined(USE_LIBICONV_DLL)
    errno = 0;
    if (libiconv_iconv_open(cd, tocode, fromcode))
        return (ppelib_iconv_t)cd;
#endif
...
int
ppelib_iconv_close(ppelib_iconv_t _cd)
{
...
size_t
ppelib_iconv(ppelib_iconv_t _cd, const char **inbuf, size_t *inbytesleft, char **outbuf, size_t *outbytesleft)
{
    rec_iconv_t *cd = (rec_iconv_t *)_cd;
...
win_iconv_open(rec_iconv_t *cd, const char *tocode, const char *fromcode)
{
    ...
    cd->iconv = win_iconv;
    cd->_errno = _errno;
    cd->cd = (ppelib_iconv_t)cd;
    return TRUE;
}

static int
win_iconv_close(ppelib_iconv_t cd UNUSED)
{
    return 0;
}

static size_t
win_iconv(ppelib_iconv_t _cd, const char **inbuf, size_t *inbytesleft, char **outbuf, size_t *outbytesleft)
{
    rec_iconv_t *cd = (rec_iconv_t *)_cd;
...
libiconv_iconv_open(rec_iconv_t *cd, const char *tocode, const char *fromcode)
{
    ...
    cd->ppelib_iconv_close = (f_iconv_close)GetProcAddressA(hlibiconv, "libiconv_close");
    if (cd->ppelib_iconv_close == NULL)
        cd->ppelib_iconv_close = (f_iconv_close)GetProcAddressA(hlibiconv, "iconv_close");
    cd->ppelib_iconv = (f_iconv)GetProcAddressA(hlibiconv, "libiconv");
    if (cd->ppelib_iconv == NULL)
        cd->ppelib_iconv = (f_iconv)GetProcAddressA(hlibiconv, "iconv");
    cd->_errno = (f_errno)find_imported_function(hlibiconv, "_errno");
    if (_iconv_open == NULL || cd->ppelib_iconv_close == NULL
            || cd->ppelib_iconv == NULL || cd->_errno == NULL)
        goto failed;

    cd->cd = _iconv_open(tocode, fromcode);
    if (cd->cd == (ppelib_iconv_t)(-1))
        goto failed;
...
main(int argc, char **argv)
{
    ...
    cd = ppelib_iconv_open(tocode, fromcode);
    if (cd == (ppelib_iconv_t)(-1))
    {
            ...
            || rest != 0)
    {
        ...
        r = ppelib_iconv(cd, &pin, &inbytesleft, &pout, &outbytesleft);
    ...
    pout = outbuf;
    outbytesleft = sizeof(outbuf);
    r = ppelib_iconv(cd, NULL, NULL, &pout, &outbytesleft);
    fwrite(outbuf, 1, sizeof(outbuf) - outbytesleft, out);
    if (r == (size_t)(-1))
    {
        perror("conversion error");
